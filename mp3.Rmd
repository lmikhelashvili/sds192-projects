...
marion_context <- tribble(
  ~year_label, ~vpos, ~hjust, ~name, ~text,
  1970, 0.75, "down", "Marion", "Marion Jones wins \ngold in Olympics"
)

jackie_context <- tribble(
  ~year_label, ~vpos, ~hjust, ~name, ~text,
1965, 0.70, "up", "Jackie", "Jackie Robinson to \nmajor league"
)
...
jamie_context <- tribble(
  ~year_label, ~vpos, ~hjust, ~name, ~text,
  1930, 0.35, "left", "Jamie", "Jamie Hunter Cartwright \nappears on Bonanza"
)
```
...
joined_data <- time_data%>%
  ...
  mutate(ordered = factor(fct_reorder(name_label, rank)))
```
...
```{r}
ggplot(joined_data, aes(x = year, y = pct_girls))+
  geom_line()+
  geom_area(fill = "#eaac9e") +
...
ggplot(aes(x = year, y = pct_girls), data = joined_data) +
  ...
  facet_wrap(~name_label) +
  geom_text(aes(x = year_label, y = vpos, label = text, hjust = hjust), data = text_data) +
  theme(
    panel.background = element_rect(fill = "#92bdd3"),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
