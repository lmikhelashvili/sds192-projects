...
marion_context <- tribble(
  ~year_label, ~vpos, ~hjust, ~name, ~text,
  2000, 0.35, "down", "Marion", "Marion Jones wins \ngold in Olympics"
)

jackie_context <- tribble(
  ~year_label, ~vpos, ~hjust, ~name, ~text,
1945, 0.35, "up", "Jackie", "Jackie Robinson to \nmajor league"
)
...
jamie_context <- tribble(
  ~year_label, ~vpos, ~hjust, ~name, ~text,
  1975, 0.35, "left", "Jamie", "Jamie Hunter Cartwright \nappears on Bonanza"
)
```
...
order_data <- all_rmses%>%
  ...
  mutate(name_label = paste(rank, ". ", name)) 
```
...
joined_data <- time_data%>%
  ...
  # mutate(ordered = fct_reorder(rank, name_label))
```

```{r}
#joined_data$name_label <- factor(joined_data$name_label)
```
...
plot_func <- function(x){
  ...
  geom_line() +
  geom_area(fill = "#eaac9e") +
  geom_point(data = year[x, ], fill = "white", pch = 21, size = 3) +
  facet_wrap(~name_label)
}
```
...
ggplot(aes(x = year, y = pct_girls), data = joined_data) +
  geom_line() +
  geom_area(fill = "#eaac9e") +
  geom_point(aes(x = year), data = year, fill = "white", pch = 21, size = 3) +
  facet_wrap(~name_label)
```
