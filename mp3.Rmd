...
```{r}
baby2 <- babynames
baby2$sex = factor(baby2$sex, levels = c("F", "M"), labels = c("Female", "Male"))
```
...
```{r}
names <- array(unique(fd_names$name))
```

```{r}
rmses <- map_df(names, rmse_function) %>%
  mutate(name = names)
```

```{r}
fd2 <- fd_names %>%
  full_join(rmses, by = "name")
```

```{r}
uni_names <- babynames %>%
  #filter(1930 <= year & year < 2012) %>%
  filter(year >= 1930 & year < 2012, n <= 10000, n > 800) %>%
  group_by(name) %>%
...
```{r}
all_names <- array(uni_names$name) 
```


```{r, eval = FALSE}
all_rmse <- map_df(all_names, rmse_function) %>%
  mutate(name = all_names)
```

```{r}
more_names <- babynames %>%
  filter(year >= 1930 & year < 2012, !is.na(n), !is.na(sex)) %>%
  group_by(name) %>%
  summarise(n = n()) %>%
  select(name)
```
...
```{r}
more_all_names <- array(more_names$name)
```


```{r}
babynames %>%
  filter(name == "ELizabeth" & sex == "M")
```

```{r}
rmse_function("Elizabeth")
```
...
```{r}
common_names <- array(uni_data$name)
```

```{r}
uni_year <- unisex_year_func(my_names)
```

```{r}
new_names <- array(my_names$name)
```

```{r}
year <- my_names %>%
  inner_join(filter_rmse, by = "name") %>%
  filter(!is.na(pct_girls), !is.na(F), !is.na(M)) %>%
  filter(name %in% my_names$name) %>%
  mutate(distance = abs(pct_girls - 0.5)) %>%
  group_by(name) %>%
