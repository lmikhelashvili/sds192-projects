...
marion_context <- tribble(
  ~year_label, ~vpos, ~hjust, ~name, ~text,
  1980, 0.30, "down", "Marion", "Marion Jones wins \ngold in Olympics"
)
...
jackie_context <- tribble(
  ~year_label, ~vpos, ~hjust, ~name, ~text,
1980, 0.35, "up", "Jackie", "Jackie Robinson to \nmajor league"
)
...
ggplot(aes(x = year, y = pct_girls), data = joined_data) +
  ...
  geom_text(aes(x = year_label, y = vpos, label = text, hjust = hjust), data = text_data, size = 2.55) +
  theme(
    panel.background = element_rect(fill = "#92bdd3"),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
